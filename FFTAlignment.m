...
function [FFTAlignmentData] = FFTAlignment(im, winsize, overlap, st, checkpoint, ...
    mask_method, maskname, figures, vectorsImageFilename, ...
    vectorsHistogramImageFilename)
...
if figures == 1
    ...
    figure('visible', 'off')
    imshow(im,[min(im(:)) max(im(:))*.8], 'Border', 'tight')
    hold on
    ...
    % change the size of the image to the original 
    drawnow
    
    % Save image with vectors to file
    imVectors = getframe (gcf);
    imwrite(imVectors.cdata, vectorsImageFilename)
    close gcf
    ...
    % Save histograms of vector orientation to file
    imVectorsDirectionHistogram = getframe (gcf);
    imwrite (imVectorsDirectionHistogram.cdata, vectorsHistogramImageFilename);
    close gcf
